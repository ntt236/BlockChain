...
function getContractAddress() {
  // Ưu tiên lấy từ biến môi trường (nếu có cấu hình file .env)
 ...
 * @param {number} courseId - ID khóa học
 * @param {string} title - Tên khóa học
 * @param {string} priceEth - Giá ETH dạng string
 * @param {string} videoUrl - Link Youtube
 * @param {string} description - Mô tả
 * @param {string} imageUrl - Ảnh đại diện
 */
export async function updateCourse(signer, courseId, title, priceEth, videoUrl, description, imageUrl) {
  const contract = getContractInstance(signer);
...
export async function getTransactionHistory(provider, userAddress) {
  try {
    ...
    const userLogs = logs.filter(log => log.args[1].toLowerCase() === userAddress.toLowerCase());
    
    const history = [];
    for (let log of userLogs) {
      const courseId = Number(log.args[0]);
      const priceWei = log.args[2];
    ...
    console.error("L?i l?y l?ch s? giao d?ch:", error);
    return [];
  }
}
